...
var gross_contribute_230 = function () {
  ...
  var custom_gross = calc_custom_make_gross(gross_info);
  var branch_gross = calc_branch_manager_make_gross(gross_info);
  var prod_gross = calc_prod_manager_make_gross(gross_info);
  var sheets = [];
  sheets.push({name: "客户毛利贡献", data: custom_gross});
  sheets.push({name: "渠道经理毛利贡献", data: branch_gross});
  sheets.push({name: "产品经理毛利贡献", data: prod_gross});
...
var calc_custom_make_gross = function( gross_info ){
  // 获取源数据中 title 的位置
  var title_array = gross_info[0];
...
// 
var calc_branch_manager_make_gross = function( gross_info ){
  // 获取源数据中 title 的位置
  var title_array = gross_info[0];
  var index_count = find_title_index(title_array, "实际交货数量");
  var index_income = find_title_index(title_array, "销售收入");
  ...
  // 查找索引
  var index_d_manager = find_title_index(title_array_dest, "渠道经理");
  var index_d_count = find_title_index(title_array_dest, "销量");
  var index_d_income = find_title_index(title_array_dest, "收入");
  var index_d_gross = find_title_index(title_array_dest, "利润");
  var index_d_avg_gross = find_title_index(title_array_dest, "单机利润贡献");
  ...
  for(var i=1; i<gross_info.length; i++){
    ...
    var manager = a_order[index_manager];
    var count = a_order[index_count];
    var income = a_order[index_income];
    var gross = a_order[index_gross];
    
    if( gross_sum[manager] ){
      // 单品汇总信息已经存在
      var temp_a = gross_sum[manager];
      // 累加数量
      temp_a[index_d_count] += count;
      // 累加收入
      temp_a[index_d_income] += income;
      ...
      // 单品汇总信息 尚未存在
      var temp_b = make_array( title_array_dest.length, "");
      temp_b[index_d_manager ] = manager;
      temp_b[index_d_count ] = count;
      temp_b[index_d_income ] = income;
      temp_b[index_d_gross ] = gross;
      //temp_b[index_d_avg_gross] = a_order[index_];   // 这三项后面再计算
...
var calc_prod_manager_make_gross = function( gross_info ){
  // 获取源数据中 title 的位置
  var title_array = gross_info[0];
  var index_count = find_title_index(title_array, "实际交货数量");
  var index_income = find_title_index(title_array, "销售收入");
  ...
  // 查找索引
  var index_d_manager = find_title_index(title_array_dest, "产品经理");
  var index_d_count = find_title_index(title_array_dest, "销量");
  var index_d_income = find_title_index(title_array_dest, "收入");
  var index_d_gross = find_title_index(title_array_dest, "利润");
  var index_d_avg_gross = find_title_index(title_array_dest, "单机利润贡献");
